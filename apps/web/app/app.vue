    ...
    <div class="app-shell min-h-screen flex flex-col">
      ...
      <header class="sticky top-0 z-50 glass border-b border-default/50 shadow-card transition-base">
        ...
        <Transition name="slide-down">
          <div v-if="mobileMenuOpen" class="md:hidden border-t border-default/50 px-4 py-3 space-y-1">
            <NuxtLink
              v-for="item in navItems"
              :key="item.to"
              :to="item.to"
              class="flex items-center gap-3 px-3 py-2.5 text-sm font-medium rounded-lg transition-base"
              :class="route.path === item.to
                ? 'text-primary bg-primary/10'
                : 'text-muted hover:text-default hover:bg-elevated'"
            >
