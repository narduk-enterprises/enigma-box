...
```bash
git clone https://github.com/narduk-enterprises/enigma-box.git my-project
cd my-project
pnpm install
...
| ---------------------- | ---------- | -------------- |
| `enigma-box` | `dev`      | Shared secrets |
| `enigma-box` | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project enigma-box --config dev_auth --no-interactive
```
